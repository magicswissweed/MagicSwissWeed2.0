/*
 * This file is generated by jOOQ.
 */
package com.aa.msw.gen.jooq;


import com.aa.msw.gen.jooq.tables.FlywaySchemaHistory;
import com.aa.msw.gen.jooq.tables.ForecastTable;
import com.aa.msw.gen.jooq.tables.HistoricalYearsDataTable;
import com.aa.msw.gen.jooq.tables.LastFewDaysSamplesTable;
import com.aa.msw.gen.jooq.tables.NotificationSubscriptionTable;
import com.aa.msw.gen.jooq.tables.SampleTable;
import com.aa.msw.gen.jooq.tables.SpotCurrentInfoTable;
import com.aa.msw.gen.jooq.tables.SpotTable;
import com.aa.msw.gen.jooq.tables.StationTable;
import com.aa.msw.gen.jooq.tables.UserTable;
import com.aa.msw.gen.jooq.tables.UserToSpotTable;
import com.aa.msw.gen.jooq.tables.records.FlywaySchemaHistoryRecord;
import com.aa.msw.gen.jooq.tables.records.ForecastTableRecord;
import com.aa.msw.gen.jooq.tables.records.HistoricalYearsDataTableRecord;
import com.aa.msw.gen.jooq.tables.records.LastFewDaysSamplesTableRecord;
import com.aa.msw.gen.jooq.tables.records.NotificationSubscriptionTableRecord;
import com.aa.msw.gen.jooq.tables.records.SampleTableRecord;
import com.aa.msw.gen.jooq.tables.records.SpotCurrentInfoTableRecord;
import com.aa.msw.gen.jooq.tables.records.SpotTableRecord;
import com.aa.msw.gen.jooq.tables.records.StationTableRecord;
import com.aa.msw.gen.jooq.tables.records.UserTableRecord;
import com.aa.msw.gen.jooq.tables.records.UserToSpotTableRecord;

import org.jooq.ForeignKey;
import org.jooq.TableField;
import org.jooq.UniqueKey;
import org.jooq.impl.DSL;
import org.jooq.impl.Internal;


/**
 * A class modelling foreign key relationships and constraints of tables in
 * public.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes", "this-escape" })
public class Keys {

    // -------------------------------------------------------------------------
    // UNIQUE and PRIMARY KEY definitions
    // -------------------------------------------------------------------------

    public static final UniqueKey<FlywaySchemaHistoryRecord> FLYWAY_SCHEMA_HISTORY_PK = Internal.createUniqueKey(FlywaySchemaHistory.FLYWAY_SCHEMA_HISTORY, DSL.name("flyway_schema_history_pk"), new TableField[] { FlywaySchemaHistory.FLYWAY_SCHEMA_HISTORY.INSTALLED_RANK }, true);
    public static final UniqueKey<ForecastTableRecord> FORECAST_TABLE_PKEY = Internal.createUniqueKey(ForecastTable.FORECAST_TABLE, DSL.name("forecast_table_pkey"), new TableField[] { ForecastTable.FORECAST_TABLE.ID }, true);
    public static final UniqueKey<ForecastTableRecord> UNIQUE_FORECAST = Internal.createUniqueKey(ForecastTable.FORECAST_TABLE, DSL.name("unique_forecast"), new TableField[] { ForecastTable.FORECAST_TABLE.TIMESTAMP, ForecastTable.FORECAST_TABLE.STATIONID, ForecastTable.FORECAST_TABLE.MEASUREMENT_TYPE }, true);
    public static final UniqueKey<HistoricalYearsDataTableRecord> HISTORICAL_YEARS_DATA_TABLE_PKEY = Internal.createUniqueKey(HistoricalYearsDataTable.HISTORICAL_YEARS_DATA_TABLE, DSL.name("historical_years_data_table_pkey"), new TableField[] { HistoricalYearsDataTable.HISTORICAL_YEARS_DATA_TABLE.DB_ID }, true);
    public static final UniqueKey<LastFewDaysSamplesTableRecord> LAST_40_DAYS_SAMPLES_TABLE_PKEY = Internal.createUniqueKey(LastFewDaysSamplesTable.LAST_FEW_DAYS_SAMPLES_TABLE, DSL.name("last_40_days_samples_table_pkey"), new TableField[] { LastFewDaysSamplesTable.LAST_FEW_DAYS_SAMPLES_TABLE.DB_ID }, true);
    public static final UniqueKey<NotificationSubscriptionTableRecord> NOTIFICATION_SUBSCRIPTION_TABLE_PKEY = Internal.createUniqueKey(NotificationSubscriptionTable.NOTIFICATION_SUBSCRIPTION_TABLE, DSL.name("notification_subscription_table_pkey"), new TableField[] { NotificationSubscriptionTable.NOTIFICATION_SUBSCRIPTION_TABLE.ID }, true);
    public static final UniqueKey<NotificationSubscriptionTableRecord> UNIQUE_USER_SUBSCRIPTION = Internal.createUniqueKey(NotificationSubscriptionTable.NOTIFICATION_SUBSCRIPTION_TABLE, DSL.name("unique_user_subscription"), new TableField[] { NotificationSubscriptionTable.NOTIFICATION_SUBSCRIPTION_TABLE.USER_ID, NotificationSubscriptionTable.NOTIFICATION_SUBSCRIPTION_TABLE.SUBSCRIPTION_TOKEN }, true);
    public static final UniqueKey<SampleTableRecord> SAMPLE_TABLE_PKEY = Internal.createUniqueKey(SampleTable.SAMPLE_TABLE, DSL.name("sample_table_pkey"), new TableField[] { SampleTable.SAMPLE_TABLE.ID }, true);
    public static final UniqueKey<SampleTableRecord> UNIQUE_SAMPLE = Internal.createUniqueKey(SampleTable.SAMPLE_TABLE, DSL.name("unique_sample"), new TableField[] { SampleTable.SAMPLE_TABLE.TIMESTAMP, SampleTable.SAMPLE_TABLE.STATIONID, SampleTable.SAMPLE_TABLE.MEASUREMENT_TYPE }, true);
    public static final UniqueKey<SpotCurrentInfoTableRecord> SPOT_CURRENT_INFO_TABLE_PKEY = Internal.createUniqueKey(SpotCurrentInfoTable.SPOT_CURRENT_INFO_TABLE, DSL.name("spot_current_info_table_pkey"), new TableField[] { SpotCurrentInfoTable.SPOT_CURRENT_INFO_TABLE.ID }, true);
    public static final UniqueKey<SpotTableRecord> SPOT_TABLE_PKEY = Internal.createUniqueKey(SpotTable.SPOT_TABLE, DSL.name("spot_table_pkey"), new TableField[] { SpotTable.SPOT_TABLE.ID }, true);
    public static final UniqueKey<StationTableRecord> STATION_TABLE_PKEY = Internal.createUniqueKey(StationTable.STATION_TABLE, DSL.name("station_table_pkey"), new TableField[] { StationTable.STATION_TABLE.DB_ID }, true);
    public static final UniqueKey<UserTableRecord> UNIQUE_EMAIL = Internal.createUniqueKey(UserTable.USER_TABLE, DSL.name("unique_email"), new TableField[] { UserTable.USER_TABLE.EMAIL }, true);
    public static final UniqueKey<UserTableRecord> UNIQUE_EXTID = Internal.createUniqueKey(UserTable.USER_TABLE, DSL.name("unique_extid"), new TableField[] { UserTable.USER_TABLE.EXTID }, true);
    public static final UniqueKey<UserTableRecord> USER_TABLE_PKEY = Internal.createUniqueKey(UserTable.USER_TABLE, DSL.name("user_table_pkey"), new TableField[] { UserTable.USER_TABLE.ID }, true);
    public static final UniqueKey<UserToSpotTableRecord> USER_TO_SPOT_TABLE_PKEY = Internal.createUniqueKey(UserToSpotTable.USER_TO_SPOT_TABLE, DSL.name("user_to_spot_table_pkey"), new TableField[] { UserToSpotTable.USER_TO_SPOT_TABLE.ID }, true);

    // -------------------------------------------------------------------------
    // FOREIGN KEY definitions
    // -------------------------------------------------------------------------

    public static final ForeignKey<NotificationSubscriptionTableRecord, UserTableRecord> NOTIFICATION_SUBSCRIPTION_TABLE__NOTIFICATION_SUBSCRIPTION_TABLE_USER_ID_FKEY = Internal.createForeignKey(NotificationSubscriptionTable.NOTIFICATION_SUBSCRIPTION_TABLE, DSL.name("notification_subscription_table_user_id_fkey"), new TableField[] { NotificationSubscriptionTable.NOTIFICATION_SUBSCRIPTION_TABLE.USER_ID }, Keys.USER_TABLE_PKEY, new TableField[] { UserTable.USER_TABLE.ID }, true);
    public static final ForeignKey<SpotCurrentInfoTableRecord, SpotTableRecord> SPOT_CURRENT_INFO_TABLE__SPOT_CURRENT_INFO_TABLE_SPOT_ID_FKEY = Internal.createForeignKey(SpotCurrentInfoTable.SPOT_CURRENT_INFO_TABLE, DSL.name("spot_current_info_table_spot_id_fkey"), new TableField[] { SpotCurrentInfoTable.SPOT_CURRENT_INFO_TABLE.SPOT_ID }, Keys.SPOT_TABLE_PKEY, new TableField[] { SpotTable.SPOT_TABLE.ID }, true);
    public static final ForeignKey<UserToSpotTableRecord, SpotTableRecord> USER_TO_SPOT_TABLE__USER_TO_SPOT_TABLE_SPOT_ID_FKEY = Internal.createForeignKey(UserToSpotTable.USER_TO_SPOT_TABLE, DSL.name("user_to_spot_table_spot_id_fkey"), new TableField[] { UserToSpotTable.USER_TO_SPOT_TABLE.SPOT_ID }, Keys.SPOT_TABLE_PKEY, new TableField[] { SpotTable.SPOT_TABLE.ID }, true);
    public static final ForeignKey<UserToSpotTableRecord, UserTableRecord> USER_TO_SPOT_TABLE__USER_TO_SPOT_TABLE_USER_ID_FKEY = Internal.createForeignKey(UserToSpotTable.USER_TO_SPOT_TABLE, DSL.name("user_to_spot_table_user_id_fkey"), new TableField[] { UserToSpotTable.USER_TO_SPOT_TABLE.USER_ID }, Keys.USER_TABLE_PKEY, new TableField[] { UserTable.USER_TABLE.ID }, true);
}
